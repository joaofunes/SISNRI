...
public class PropuestaConvenioDao extends GenericDao<PropuestaConvenio, Integer> {
    ...
    public List<PropuestaConvenio> getConveniosMarcos() {
        try {
            String sql = "SELECT PRO.ID_PROPUESTA idPropuesta,NOMBRE_PROPUESTA nombrePropuesta,FINALIDAD_PROPUESTA finalidadPropuesta,VIGENCIA vigencia,ID_TIPO_PROPUESTA_CONVENIO\n"
                    ...
                    + "WHERE PRO.VIGENCIA IS NOT NULL\n"
                    + "AND ESTADO.NOMBRE_ESTADO='FIRMADO' "
                    + "AND PRO.ID_TIPO_PROPUESTA_CONVENIO=1 "
                    + "AND PRO.ACTIVO IS TRUE";
    ...
    public List<PropuestaConvenio> getConveniosMarcosEspecifico() {
        try {
            String sql = "SELECT PRO.ID_PROPUESTA idPropuesta,NOMBRE_PROPUESTA nombrePropuesta,FINALIDAD_PROPUESTA finalidadPropuesta,VIGENCIA vigencia,ID_TIPO_PROPUESTA_CONVENIO\n"
                    ...
                    + "WHERE PRO.VIGENCIA IS NOT NULL\n"
                    + "AND ESTADO.NOMBRE_ESTADO='FIRMADO' "
                    + "AND PRO.ID_TIPO_PROPUESTA_CONVENIO IN (1,2) "
                    + "AND PRO.ACTIVO IS TRUE";
    ...
    public List<PojoPropuestaConvenio> getAllPropuestaConvenioSQL() {
        ...
        String sql = "SELECT * FROM  \n"
                ...
                + "AND STA.NOMBRE_ESTADO NOT IN ('FIRMADO','RECHAZADA')"
    ...
    /**
     * obtiene todos las propuestas convenios canceladas SQL sin id solicitante
     *
     * @return
     */
    public List<PojoPropuestaConvenio> getAllPropuestaCanceladasConvenioSQL() {
        ...
        String sql = "SELECT * FROM  \n"
                ...
                + "ON P_ESTADO.ID_ESTADO=STA.ID_ESTADO\n"
                + "WHERE  P_CONVENIO.VIGENCIA IS NULL\n"
                + "AND STA.NOMBRE_ESTADO IN ('RECHAZADA')"
                + "ORDER BY FECHA_INGRESO DESC ) TB_CONVENIO\n"
                + "\n"
                + "LEFT JOIN\n"
                + "\n"
     ...
     * obtiene todos las propuestas convenios SQL por id solicitante
     */
    public List<PojoPropuestaConvenio> getAllPropuestaConveniCanceladasSQL(int idSolicitante) {
    ...
    public List<RptConteoConveniosPorTipoPojo> conteoPropuestaConvenioByTipoPropuesta(Integer desde, Integer hasta) {
        try {
            String sql = "select tipo.NOMBRE_PROPUESTA_CONVENIO as tipoConvenio, count(pc.ID_PROPUESTA) as suma from PROPUESTA_CONVENIO pc join TIPO_PROPUESTA_CONVENIO tipo on (pc.ID_TIPO_PROPUESTA_CONVENIO=tipo.ID_TIPO_PROPUESTA) WHERE YEAR(pc.FECHA_INGRESO) BETWEEN \n"
