...
public class BecaDao extends GenericDao<Beca, Integer> {
    ...
    public List<PojoBeca> getBecas(Integer idBecaSearch) {
        String query = "SELECT bec.ID_BECA idBeca,\n"
                ...
                + "  fac.NOMBRE_FACULTAD as facultad,\n"
                + "  per.APELLIDO_PERSONA apellidoBecario,\n"
                + "pai.NOMBRE_PAIS paisDestino,\n"
                + "  org.NOMBRE_ORGANISMO universidadDestino,\n"
                + "  bec.MONTO_TOTAL montoBeca,IF(bec.OTORGADA = 1, 'SI','NO') as  otorgada\n"
                + "FROM beca bec\n"
                ...
                + "  ON ca.ID_CARRERA=per.ID_CARRERA\n"
        ...
        query = query + " ORDER BY bec.FECHA_INGRESO DESC;";
